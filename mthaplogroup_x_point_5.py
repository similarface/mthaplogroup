...
def getMthaologroup(mttree, sdata, result,weightTree):
    ...
    if currentfenmuweight!=0:
        result[id] = {'pid': pid, 'name': name, 'matchrsid': matchrsid, 'nomathrsid': nomathrsid,
                  'weight': (0.5*currentfenziweight/currentfenmuweight+0.5*currentfenziweight/weightTree),
                       "qfeziweight":currentfenziweight,
                  "qfenmuweight":currentfenmuweight}
    else:
        result[id] = {'pid': pid, 'name': name, 'matchrsid': matchrsid, 'nomathrsid': nomathrsid,
                  'weight': 0+0.5*currentfenziweight/weightTree,
                       "qfeziweight":currentfenziweight,
                  "qfenmuweight":currentfenmuweight}
...
def getPath(mttree, deques, id):
    ...
    # 左侧加入该节点的父亲节点
    try:
        deques.appendleft({mttree[id]['name']:{'matchrsid':mttree[id]['matchrsid'],'nomatchrsid':mttree[id]['nomathrsid'],'weight':mttree[id]['weight']}})
    except Exception,e:
        print(id)
    # 获取ytree树的父节点的PID
    pid = mttree[id]['pid']
...
def getMthaologroupPath(sdata,weightTree):
    ...
    for node in getMaxWeightNode(result):
        ...
        for item in treepath:
            for k,v in item.items():
                print(k,v['matchrsid'],v['nomatchrsid'],v['weight'])
    ...
    print("x0.5\t"+getFormatYhaologroupPath(treepath)[-1])
    return retreepath
...
if __name__ == "__main__":
    filename=filename='/Users/similarface/Documents/user3172_file2057_yearofbirth_unknown_sex_unknown.23andme.txt'
    if len(sys.argv)!=1:
