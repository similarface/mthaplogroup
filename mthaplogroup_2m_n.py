...
def getMthaologroup(mttree, sdata, result,weightTree):
    ...
    for rsid in rsids:
        try:
            ...
            #nomathrsid.append(rsid)
            try:
                logger.debug("pos {0},mutation {1},name {2} ,match {3}".format(rsid, '?', name,'UNKOWNU'))
            except KeyError, e:
                try:
                    logger.debug(
    ...
    weight=result[pid]['weight']+(len(matchrsid)-len(nomathrsid))
    result[id] = {'pid': pid, 'name': name, 'matchrsid': matchrsid, 'nomathrsid': nomathrsid,'weight': weight}
    if len(children) != 0:
...
def getMthaologroupPath(sdata,weightTree):
    ...
    print("2m-n\t"+getFormatYhaologroupPath(treepath)[-1])
    return retreepath
...
if __name__ == "__main__":
    ...
    filename=filename='/Users/similarface/Documents/FangGE.txt'
    if len(sys.argv)!=1:
